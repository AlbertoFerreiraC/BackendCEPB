...
class InscripcionAdmin(admin.ModelAdmin):
    list_display = ('id', 'tutor_alumno', 'ins_fecha', 'ins_descuento', 'ins_contrato_fecha', 'generar_pagare_pdf_link', 'generar_ficha_pdf_link')
    search_fields = ('tutor_alumno__tutor__tut_nom', 'tutor_alumno__alumno__alum_nom')
    list_filter = ('ins_fecha', 'ins_descuento')
    actions = ['generar_pdf_action', 'generar_pagare_pdf_action', 'generar_ficha_pdf_action']
    ...
    def generar_ficha_pdf_action(self, request, queryset):
        if queryset.count() != 1:
            self.message_user(request, "Por favor seleccione exactamente una inscripción.")
            return

        inscripcion_id = queryset.first().id
        return HttpResponseRedirect(reverse('genera_ficha_pdf', kwargs={'id': inscripcion_id}))
    generar_ficha_pdf_action.short_description = "Generar PDF de la ficha"

    def generar_ficha_pdf_link(self, obj):
        url = reverse('genera_ficha_pdf', args=[obj.id])
        return format_html('<a href="{}" target="_blank">Generar ficha PDF</a>', url)
    generar_ficha_pdf_link.short_description = 'Generar ficha PDF'
