...
class Inscripcion(models.Model):
    tutor_alumno = models.ForeignKey(TutorAlumno, on_delete=models.CASCADE)
...
class Arancel(models.Model):
    inscripcion = models.ForeignKey('Inscripcion', related_name='aranceles', on_delete=models.CASCADE)
    arancel_nivel = models.CharField(max_length=20, choices=[
        ('Escolar Básica', 'Escolar Básica'), 
        ('Nivel Inicial', 'Nivel Inicial'), 
        ('Educación Media', 'Educación Media')
    ])
    arancel_ciclo = models.CharField(max_length=20, choices=[
    ...
    arancel_especializacion = models.CharField(max_length=20, choices=[
        ('BTI', 'BTI'), 
        ('BTA', 'BTA'), 
        ('Ciencias Sociales', 'Ciencias Sociales'), 
        ('Ciencias Básicas', 'Ciencias Básicas')
    ], null=True, blank=True)
    arancel_grado = models.CharField(max_length=20, choices=[
        ...
        ('3º Bachillerato', '3º Bachillerato')
    ], null=True, blank=True)
    arancel_turno = models.CharField(max_length=20, choices=[
        ('Mañana', 'Mañana'), 
        ('Tarde', 'Tarde')
    ], null=True, blank=True)
